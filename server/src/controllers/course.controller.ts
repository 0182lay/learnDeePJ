...
export const createCourse = async (req: Request, res: Response) => {
    try {
        ...
        const { category_id, title, description, price, thumbnail_url, level, is_published } =
            req.body;
...
export const getMyCourses = async (req: Request, res: Response) => {
    try {
        ...
        const courses = await getMyCoursesService(instructor_id, userRole);

        res.status(200).json({
            message: "ດຶງຂໍ້ມູນຄອສຂອງຂ້ອຍສຳເລັດ",
            data: courses,
        });
    } catch (error) {
        console.log(error);
        res.status(500).json({ message: "ດຶງຂໍ້ມູນຄອສຂອງຂ້ອຍບໍ່ສຳເລັດ" });
    }
};

export const uploadCourseCover = async (req: Request, res: Response) => {
    try {
        const uploadedFile = req.file;

        if (!uploadedFile) {
            return res.status(400).json({
                message: "ກະລຸນາອັບໂຫຼດຮູບປົກ",
            });
        }

        return res.status(201).json({
            message: "ອັບໂຫຼດຮູບປົກສຳເລັດ",
            data: {
        ...
        return res.status(500).json({
            message: "ອັບໂຫຼດຮູບປົກບໍ່ສຳເລັດ",
        });
    }
};
